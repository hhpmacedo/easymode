...
import type { ExtractionMemory, ExtractionTurn } from "@/lib/prompts/memory";
...
// clipping below. clipTurns, clipMemories and the extract limiter (20 per
// 15 min) bound what reaches Haiku, not this cap.
...
export async function POST(req: Request) {
  ...
  const rawMemories = Array.isArray(body.memories) ? (body.memories as ExtractionMemory[]) : [];
  const memoriesOk = rawMemories.every(
    (m) =>
      m &&
      typeof m.id === "string" &&
  ...
  const memories = clipMemories(rawMemories);
